...
func _ready() -> void:
	label = $DefeatLayer/Label
	label.modulate.a = 0.0
	
	# 从文件读取失败文字
	_load_defeat_from_file()
...
func _load_defeat_from_file() -> void:
	"""从defeat.txt文件读取失败内容"""
...
func _show_current_text() -> void:
	if current_text_index < defeat_texts.size():
		label.text = defeat_texts[current_text_index]
	else:
		# 所有文字显示完毕，准备返回主菜单
		_finish_defeat()
